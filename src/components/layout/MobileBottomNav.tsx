...
export const MobileBottomNav: React.FC<MobileBottomNavProps> = ({ activeTab, onSelectTab }) => {
  ...
  return (
    <nav
      ...
      className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-xl border-t border-teal-900/10 shadow-[0_-4px_20px_rgba(16,37,71,0.08)] px-2 pt-2 pb-[max(0.5rem,env(safe-area-inset-bottom))]"
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center justify-center py-1.5 px-1 rounded-xl transition-all duration-200 cursor-pointer ${
                isActive
                  ? 'text-[#0D9488] font-bold bg-teal-50'
            ...
            >
              <div className="relative">
                ...
                {isActive && (
                  <span className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-1.5 h-1.5 rounded-full bg-[#0D9488]" />
                )}
              </div>
